...
async function openProfileModal(userId) {
    ...
    setTimeout(() => {
        const firstTab = modal.querySelector('.profile-tab');
...
async function loadUserProfile(userId) {
    try {
        ...
        document.getElementById('profileBio').textContent = 'Loading profile...';
        
        // Fetch user profile data
        const response = await fetch(`${API_BASE_URL}/get_user_profile.php?user_id=${userId}`, {
            credentials: 'include'
        });
        ...
        if (data.success) {
            ...
            document.getElementById('ratingValue').textContent = parseFloat(user.rating || 0).toFixed(1);
            document.getElementById('profileBio').textContent = user.bio || 'This user hasn\'t added a bio yet.';
            
            // Update About tab
            document.getElementById('totalExchanges').textContent = user.total_exchanges || '0';
            document.getElementById('memberSince').textContent = user.member_since ? 
                new Date(user.member_since).getFullYear() : '2025';
            document.getElementById('aboutRating').textContent = parseFloat(user.rating || 0).toFixed(1) + ' ⭐';
            document.getElementById('userStatus').textContent = user.is_active ? 'Active' : 'Inactive';
            
            // Update Skills tab
            const offeredSkillsList = document.getElementById('offeredSkillsList');
            if (user.offered_skills && user.offered_skills.length > 0) {
                offeredSkillsList.innerHTML = user.offered_skills.map(skill => `
                    <span class="skill-tag level-${skill.proficiency_level || 'beginner'}">
            ...
            if (user.learning_skills && user.learning_skills.length > 0) {
                learningSkillsList.innerHTML = user.learning_skills.map(skill => `
                    <span class="skill-tag">
                        ${escapeHtml(skill.skill_name)}
                    </span>
                `).join('');
            } else {
                learningSkillsList.innerHTML = `
                    <div class="empty-state-small">
                        <div class="empty-state-icon">🎯</div>
...
// Connect from profile modal
function connectFromProfile() {
    if (currentProfileUser) {
        const skillCard = skillsData.find(s => s.user_id === currentProfileUser);
        if (skillCard) {
            closeProfileModal();
            setTimeout(() => {
                openConnectModal(skillCard);
